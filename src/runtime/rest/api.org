...
** Run test cases
  ...
  This Class contains the function which runs all the test cases
...
def runTestCases(git_repo_url):
    current_app.logger.debug("git_repo_url: %s" % git_repo_url)
    try:
        test_files = subprocess.Popen(["find . -name '*test*.py'"], shell=True, stdout=subprocess.PIPE).stdout
	test_files_paths = test_files.read().splitlines()
	count = 0
	for test_file in test_files_paths:
            command = "/usr/bin/python %s" % test_file
            (ret_code, output) = linuxCommandExecuter(command)
	    if ret_code == 0:
	        count = count + 1
	if count == len(test_files_path):	
	    title = "Successfully ran all the test cases"
            labels = ["build_passed"]
        else:
	    title = "Failed to run test cases"
	    labels = ["build_failed"]

	(repo_name, repo_org) = repoNameExtacter(git_repo_url)
...
*** Unit Test for running test cases
...
class TestRunCases(unittest.TestCase):
    ...
    def test_run_cases(self):
        ...
        with app.app_context():
            print "test_build_repo"
            url = "https://github.com/vlead/lab-data-service"
            self.assertEqual(runTestCases(url), True)
...
def build_repo():
    ...
    if request.method == 'POST':
        """
        req_url = request.url
        if ("vlead" not in req_url) or ("virtual-labs" not in req_url):
            #current_app.logger.error("Git repo = %s, organization = %s are not authorized to invoke web hook" % (repo_name, repo_org))
            abort(401, "Unauthorized repository/organization")
        """
	try:
	...
	build_status = repo_build.repoBuilder()
	tests_status = runTestCases(git_url)
	if build_status and tests_status:
	    message = {
		      "status" : 200,
